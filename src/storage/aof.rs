...
impl Aof {
    pub async fn new(path: &str, store: Arc<Mutex<Store>>) -> Result<Self, std::io::Error> {
        ...
        let aof = Self { state, path: path.to_string() };
    ...
    fn restore(data: &[u8], store: &Arc<Mutex<Store>>) {
        ...
        while i < data.len() {
            ...
            for _ in 0..3 {
                ...
                let part = &data[i..i + len];
                parts.push(part);
                i += len + 2; // skip data and \r\n
            }
    ...
    pub fn append(&self, cmd: &Command) -> Result<(), String> {
        if let Some(data) = self.serialize_command(cmd) {
            let mut state = self.state.lock().map_err(|_| "AOF mutex poisoned".to_string())?;
    ...
    pub async fn flush_now(&self) -> Result<(), std::io::Error> {
        let (buf, file_opt) = {
            let mut state = self.state.lock().map_err(|_| std::io::Error::new(std::io::ErrorKind::Other, "mutex poisoned"))?;
            let buf = std::mem::take(&mut state.buffer);
            let file = state.file.take();
            (buf, file)
        };
    ...
    pub async fn rewrite(&self, store: Arc<Mutex<Store>>) -> Result<(), std::io::Error> {
        ...
        // 1. Flush current buffer synchronously to disk
        ...
        // 3. Initialize rewrite buffer to collect new writes during rewrite
        {
